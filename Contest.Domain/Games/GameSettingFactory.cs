...
namespace Contest.Domain.Games
{
    ...
    public class GameSettingFactory : IGameSettingFactory
    {
        ...
        {
            if (minimumPlayerByTeam < 1)
                throw new ArgumentException("Une équipe doit au moins pouvoir posséder un joueur");
            if (maximumPlayerByTeam < minimumPlayerByTeam)
                throw new ArgumentException(
                    $"Le nombre maximum de joueur par équipe ne peut pas être inférieur au nombre minimum. Nombre min:{minimumPlayerByTeam}. Nombre Max:{maximumPlayerByTeam}");
